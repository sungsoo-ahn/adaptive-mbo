...
class Discriminator(tf.keras.Model):
    ...
    def loss(self, x, y, labels, **kwargs):
        """Use a neural network to discriminate the log probability that a
        sampled design X has score y
        ...
        if self.method == "wasserstein":
            ...
            return (
                x,
                p,
                tf.where(labels > 0.5, tf.cast(x > 0.0, tf.float32), tf.cast(x < 0.0, tf.float32)),
            )
        ...
        # Implements the Least-Squares GAN loss function
            ...
            return (
                x,
                p,
                tf.where(labels > 0.5, tf.cast(x > 0.5, tf.float32), tf.cast(x < 0.5, tf.float32)),
            )
...
class DiscreteGenerator(tf.keras.Model):
    ...
    def sample(self, y, **kwargs):
        ...
        return tfpd.RelaxedOneHotCategorical(temp, logits=tf.math.log_softmax(logits)).sample()
