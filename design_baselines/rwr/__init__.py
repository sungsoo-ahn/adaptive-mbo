...
from design_baselines.rwr.trainers import Ensemble
from design_baselines.rwr.trainers import WeightedVAE
from design_baselines.rwr.trainers import RWR
from design_baselines.rwr.nets import ForwardModel
from design_baselines.rwr.nets import Encoder
from design_baselines.rwr.nets import DiscreteDecoder
from design_baselines.rwr.nets import ContinuousDecoder
...
def rwr(config):
    ...
    p_encoder = Encoder(task.input_shape, config["latent_size"], hidden=config["hidden_size"])
    p_decoder = decoder(task.input_shape, config["latent_size"], hidden=config["hidden_size"])
    p_vae = WeightedVAE(
        p_encoder,
        p_decoder,
    ...
    p_manager = tf.train.CheckpointManager(
        ...
        os.path.join(config["logging_dir"], "p_vae"),
        1,
    )
    ...
    p_vae.launch(train_data, val_data, logger, config["offline_epochs"])

    # build the encoder and decoder distribution and the p model
    q_encoder = Encoder(task.input_shape, config["latent_size"], hidden=config["hidden_size"])
    q_decoder = decoder(task.input_shape, config["latent_size"], hidden=config["hidden_size"])
    q_vae = WeightedVAE(
        q_encoder,
        q_decoder,
        vae_optim=tf.keras.optimizers.Adam,
    ...
    # create the rwr importance weight generator
    rwr = RWR(ensemble, p_vae, q_vae, latent_size=config["latent_size"])
    ...
    for i in range(config["iterations"]):
        ...
        x, y, w = rwr.generate_data(
            config["online_batches"], config["vae_batch_size"], config["alpha"]
        )
        ...
        q_vae.launch(train_data, val_data, logger, config["online_epochs"], start_epoch=start_epoch)
