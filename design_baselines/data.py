...
class StaticGraphTask(Task):
    ...
    def build(
        ...
        if bootstraps > 1:
            ...
            train_inputs.append(
                tf.stack(
                    [
                        tf.math.bincount(
                            tf.random.uniform([size], minval=0, maxval=size, dtype=tf.int32),
                            minlength=size,
                            dtype=tf.float32,
                        )
            ...
            if bootstraps_noise is not None:
                train_inputs.append(bootstraps_noise * tf.random.normal([size, bootstraps]))

        if bootstraps == 1:
            train_inputs.append(tf.ones([size, 1]))
        ...
        if val_size > 0:
            ...
            return train.prefetch(tf.data.experimental.AUTOTUNE), None
