...
def reinforce(config):
    ...
    rl_opt = tf.keras.optimizers.Adam(learning_rate=config["reinforce_lr"])
    ...
    for iteration in range(config["iterations"]):
        ...
        logger.record("reinforce/prediction", ty, iteration, percentile=True)
        logger.record("reinforce/loss", loss, iteration, percentile=True)
        print(f"[Iteration {iteration}] " f"Average Score = {tf.reduce_mean(ty)}")

        grads = tape.gradient(loss, sampler.trainable_variables)

        rl_opt.apply_gradients(zip(grads, sampler.trainable_variables))
