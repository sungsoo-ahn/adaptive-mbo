...
class Logger(object):
    ...
    def record(self, key, value, step, percentile=False, report=False):
        ...
        with self.writer.as_default():
            ...
            if tf.equal(tf.size(value), 1):
                ...
                tf.summary.scalar(key + "/100th", tfp.stats.percentile(value, 100.0), step=step)
                tf.summary.scalar(key + "/90th", tfp.stats.percentile(value, 90.0), step=step)
                tf.summary.scalar(key + "/80th", tfp.stats.percentile(value, 80.0), step=step)
                tf.summary.scalar(key + "/50th", tfp.stats.percentile(value, 50.0), step=step)

            else:

                # log several statistics of the incoming tensors
                tf.summary.scalar(key + "/max", tf.math.reduce_max(value), step=step)
                tf.summary.scalar(key + "/mean", tf.math.reduce_mean(value), step=step)
                tf.summary.scalar(key + "/min", tf.math.reduce_min(value), step=step)
                tf.summary.scalar(key + "/std", tf.math.reduce_std(value), step=step)
