...
def autofocus(config):
    ...
    cbas = CBAS(ensemble, p_vae, q_vae, latent_size=config["latent_size"])
    ...
    for i in range(config["iterations"]):
        ...
        x_t, y_t, w = cbas.generate_data(
            config["online_batches"], config["vae_batch_size"], config["percentile"]
        )
        ...
        v = cbas.autofocus_weights(x, batch_size=config["ensemble_batch_size"])
        train_data, val_data = task.build(
            x=x,
            y=y,
            bootstraps=config["bootstraps"],
        ...
        # train a vae fit using weighted maximum likelihood
        start_epoch = config["autofocus_epochs"] * i + config["ensemble_epochs"]
        ensemble.launch(
            train_data, val_data, logger, config["autofocus_epochs"], start_epoch=start_epoch
        )
