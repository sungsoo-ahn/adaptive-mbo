        ...
        for i in range(self.bootstraps):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                total_loss = tf.math.divide_no_nan(
                    tf.reduce_sum(w[:, 0] * b[:, i] * nll), tf.reduce_sum(b[:, i])
                )
...
class CBAS(tf.Module):
    ...
    @tf.function(experimental_relax_shapes=True)
    def generate_data(self, num_batches, num_samples, percentile):
        ...
        for j in range(num_batches):
            ...
            log_w = p_dx.log_prob(x)[..., tf.newaxis] - q_dx.log_prob(x)[..., tf.newaxis]
        ...
        return tf.concat(xs, axis=0), tf.concat(ys, axis=0), tf.concat(ws, axis=0)
    ...
    def get_autofocus_ratio(self, x):
        ...
        # evaluate the score and importance weights
        log_w = q_dx.log_prob(x)[..., tf.newaxis] - p_dx.log_prob(x)[..., tf.newaxis]
        while len(log_w.shape) > 2:
            log_w = tf.reduce_sum(log_w, axis=1)
