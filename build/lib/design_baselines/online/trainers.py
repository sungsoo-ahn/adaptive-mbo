...
class ConservativeMaximumLikelihood(tf.Module):
    ...
    @tf.function(experimental_relax_shapes=True)
    def lookahead(self, x, steps, **kwargs):
    ...
    def train_step(self, x, y):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            x_pos = tf.where(
                tf.random.uniform([batch_dim] + [1 for _ in x.shape[1:]]) < self.negatives_fraction,
                x_pos,
                self.solution[:batch_dim],
            )
        ...
        if tf.logical_and(
            ...
            update = self.lookahead(self.solution, self.solver_steps, training=False)
