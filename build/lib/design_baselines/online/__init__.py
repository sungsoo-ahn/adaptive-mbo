...
def online(config):
    ...
    trainer.done = tf.Variable(tf.fill([config["batch_size"]] + [1 for _ in x.shape[1:]], False))
    ...
    def evaluate_solution(xt):
        ...
        logger.record(f"oracle/max_of_mean", max_of_mean, evaluations)
        logger.record(f"oracle/max_of_stddev", max_of_stddev, evaluations)
        logger.record(f"oracle/min_of_mean", min_of_mean, evaluations)
        logger.record(f"oracle/min_of_stddev", min_of_stddev, evaluations)
        logger.record(f"oracle/mean_of_mean", mean_of_mean, evaluations)
        logger.record(f"oracle/mean_of_stddev", mean_of_stddev, evaluations)

        # record the prediction and score to the logger
        logger.record("score", score, evaluations, percentile=True)
        logger.record("distance/travelled", tf.linalg.norm(solution - initial_x), evaluations)
        logger.record("distance/from_mean", tf.linalg.norm(solution - mean_x), evaluations)
        logger.record(f"oracle/prediction", model, evaluations)
        logger.record(
            f"oracle/grad_norm",
            tf.linalg.norm(tf.reshape(grads, [-1, task.input_size]), axis=-1),
            evaluations,
        )
        logger.record(f"rank_corr/model_to_real", spearman(model[:, 0], score[:, 0]), evaluations)
