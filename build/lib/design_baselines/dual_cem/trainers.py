...
class Ensemble(tf.Module):
    def __init__(
        self, forward_models, forward_model_optim=tf.keras.optimizers.Adam, forward_model_lr=0.001
    ):
...
class WeightedVAE(tf.Module):
    def __init__(
        self, encoder, decoder, vae_optim=tf.keras.optimizers.Adam, vae_lr=0.001, vae_beta=1.0
    ):
        """Build a trainer for an ensemble of probabilistic neural networks
        trained on bootstraps of a dataset
    ...
    def train_step(self, x, y, w):
        ...
        with tf.GradientTape() as tape:
            ...
            prior = tfpd.MultivariateNormalDiag(loc=tf.zeros_like(z), scale_diag=tf.ones_like(z))
...
class DualCEM(tf.Module):
    def __init__(self, ensemble, vaes, vae_beta, latent_size=20):
        super().__init__()
    ...
    @tf.function(experimental_relax_shapes=True)
    def compute_gamma(self, num_samples, gamma_percentile):
        #print(num_samples, gamma_percentile)
        z = tf.random.normal([num_samples, self.latent_size])
        x = self.vaes[0].decoder.get_distribution(z, training=False).sample()

        dz = self.vaes[1].encoder.get_distribution(x, training=False)
        z = dz.sample()
        dx = self.vaes[1].decoder.get_distribution(z, training=False)
        nll = -dx.log_prob(x)
        while len(nll.shape) > 1:
            nll = tf.reduce_sum(nll, axis=1)

        prior = tfpd.MultivariateNormalDiag(loc=tf.zeros_like(z), scale_diag=tf.ones_like(z))
        kl = dz.kl_divergence(prior)
        elbo = - (nll + self.vae_beta * kl)

        gamma = tfp.stats.percentile(elbo, gamma_percentile)
        return gamma

    @tf.function(experimental_relax_shapes=True)
    def generate_data(self, num_batches, num_samples, alpha, gamma):

        x_pair = []
        y_pair = []
        w_pair = []

        for vae0, vae1 in ((self.vaes[0], self.vaes[1]), (self.vaes[1], self.vaes[0])):
            ...
            for j in range(num_batches):
                ...
                z = tf.random.normal([num_samples, self.latent_size])
                x = vae0.decoder.get_distribution(z, training=False).sample()
                y = self.ensemble.get_distribution(x).mean()
                w = tf.math.exp(alpha*y)

                dz = vae1.encoder.get_distribution(x, training=False)
                z = dz.sample()
                dx = vae1.decoder.get_distribution(z, training=False)
                nll = -dx.log_prob(x)
                while len(nll.shape) > 1:
                    nll = tf.reduce_sum(nll, axis=1)
