...
def get_rank(x):
    return tf.cast(tf.argsort(tf.argsort(x, axis=-1, direction="ASCENDING"), axis=-1) + 1, x.dtype)
...
def spearman(a, b):
    ...
    x = get_rank(a)
    y = get_rank(b)
    cov = tfp.stats.covariance(x, y, sample_axis=-1, keepdims=False, event_axis=None)
    sd_x = tfp.stats.stddev(x, sample_axis=-1, keepdims=True, name=None)
    sd_y = tfp.stats.stddev(y, sample_axis=-1, keepdims=True, name=None)
...
def generate_ensemble(num_layers, *activations):
    ...
    return [
        [act, *o] for act in activations for o in generate_ensemble(num_layers - 1, *activations)
    ]
...
def render_video(config, task, solution):
    ...
    if config["task"] == "HopperController-v0":
        ...
        out = FFmpegWriter(os.path.join(config["logging_dir"], f"vid.mp4"))
        ...
        for i in range(5):
            ...
            while not done:
                ...
                out.writeFrame(env.render(mode="rgb_array", height=500, width=500))
