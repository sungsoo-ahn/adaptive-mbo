...
def gradient_ascent(config):
    ...
    score = 0.0#task.score(solution * st_x + mu_x)
    ...
    for i in range(1, config["solver_steps"] + 1):
        ...
        held_out_m = [m.get_distribution(solution).mean() for m in held_out_models]
        held_out_s = [m.get_distribution(solution).stddev() for m in held_out_models]
        ...
        logger.record(f"held_out/max_of_mean", max_of_mean, i)
        logger.record(f"held_out/max_of_stddev", max_of_stddev, i)
        logger.record(f"held_out/min_of_mean", min_of_mean, i)
        logger.record(f"held_out/min_of_stddev", min_of_stddev, i)
        logger.record(f"held_out/mean_of_mean", mean_of_mean, i)
        logger.record(f"held_out/mean_of_stddev", mean_of_stddev, i)
        ...
        for n, prediction_i in enumerate(preds):
            ...
            if n > 0:
                logger.record(
                    f"rank_corr/0_to_{n}", spearman(preds[0][:, 0], prediction_i[:, 0]), i
                )
                logger.record(
                    f"grad_corr/0_to_{n}",
                    tfp.stats.correlation(grads[0], grads[n], sample_axis=0, event_axis=None),
                    i,
                )
...
def ablate_architecture(config):
    ...
    # create a trainer for a forward model with a conservative objective
    held_out_trainer = Ensemble(
        held_out_models, forward_model_optim=tf.keras.optimizers.Adam, forward_model_lr=0.001
    )
    ...
    def evaluate_solution(solution, evaluations):
        ...
        logger.record(f"oracle/same_architecture/max_of_mean", max_of_mean, evaluations)
        logger.record(f"oracle/same_architecture/max_of_stddev", max_of_stddev, evaluations)
        logger.record(f"oracle/same_architecture/min_of_mean", min_of_mean, evaluations)
        logger.record(f"oracle/same_architecture/min_of_stddev", min_of_stddev, evaluations)
        logger.record(f"oracle/same_architecture/mean_of_mean", mean_of_mean, evaluations)
        logger.record(f"oracle/same_architecture/mean_of_stddev", mean_of_stddev, evaluations)

    # train a held-out model on the validation set

    for file_name in glob.glob(os.path.join(config["logging_dir"], f"solution_*.npy")):
