...
def get_p_y(scores, base_temp=None):
    ...
    return provable_dist[:, np.newaxis], bin_edges[:, np.newaxis]


@tf.function(experimental_relax_shapes=True)
def get_synthetic_data(x, y, exploration_samples=32, exploration_rate=10.0, base_temp=None):
    ...
    return tf.concat([x, xs], 0), tf.concat([y, ys], 0)
