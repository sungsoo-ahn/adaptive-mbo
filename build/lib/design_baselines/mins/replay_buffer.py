...
class ReplayBuffer(tf.Module):
    def __init__(self, capacity, shape):
        ...
        self.xs = tf.Variable(tf.zeros([capacity, *shape], tf.dtypes.float32))
        self.ys = tf.Variable(tf.zeros([capacity, 1], tf.dtypes.float32))

        # a tensor for indexing into scatter_nd_update
        self.idx = tf.concat(
            [
                tf.tile(
                    tf.reshape(tf.range(shape_i), [1] * i + [shape_i] + [1] * (len(shape) - i)),
    ...
    def insert(self, x, y):
        ...
        y: tf.dtypes.float32
            a tensor corresponding to a fake scores from the generator
            images may be shaped like [1]
        """
        ...
        loc_y = tf.concat([tf.reshape(self.head, [1, 1]), [[0]]], axis=1)

        # insert samples at the position of the current head
        self.xs.assign(tf.tensor_scatter_nd_update(self.xs, loc_x, tf.cast(x, tf.dtypes.float32)))
        self.ys.assign(tf.tensor_scatter_nd_update(self.ys, loc_y, tf.cast(y, tf.dtypes.float32)))
    ...
    def insert_many(self, xs, ys):
        ...
        ys: tf.dtypes.float32
            a tensor corresponding to a fake scores from the generator
            images may be shaped like [batch, 1]
        """

        for i in tf.range(tf.shape(xs)[0]):
            self.insert(xs[i], ys[i])
    ...
    def sample(self, batch_size):
        ...
        indices = tf.random.uniform([batch_size], maxval=self.size, dtype=tf.dtypes.int32)
        return tf.gather(self.xs, indices, axis=0), tf.gather(self.ys, indices, axis=0)
