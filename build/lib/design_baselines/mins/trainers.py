...
class Ensemble(tf.Module):
    def __init__(
        ...
        self.forward_model_optims = [
            forward_model_optim(learning_rate=forward_model_lr) for _ in range(self.bootstraps)
        ]
    ...
    def train_step(self, x, y, b):
        ...
        for i in range(self.bootstraps):
            ...
            optim.apply_gradients(zip(tape.gradient(total_loss, var_list), var_list))
...
class WeightedGAN(tf.Module):
    def __init__(
        ...
        self.generator_optim = tf.keras.optimizers.Adam(
            learning_rate=generator_lr, beta_1=generator_beta_1, beta_2=generator_beta_2
        )

        # create optimizers for the discriminator
        self.discriminator = discriminator
        self.discriminator_optim = tf.keras.optimizers.Adam(
            learning_rate=discriminator_lr, beta_1=discriminator_beta_1, beta_2=discriminator_beta_2
        )
    ...
    def train_step(self, i, x_real, y_real, w):
        ...
        with tf.GradientTape() as tape:
            ...
            if self.pool_save > 0:
                ...
                self.pool.insert_many(x_fake[: self.pool_save], y_real[: self.pool_save])

            # evaluate the discriminator on real inputs
            labels = tf.cast(self.flip_frac <= tf.random.uniform([batch_dim, 1]), tf.float32)
            ...
            # build the total loss
            total_loss = tf.reduce_mean(w * (d_real + d_fake + self.penalty_weight * penalty))

        var_list = self.discriminator.trainable_variables
        self.discriminator_optim.apply_gradients(zip(tape.gradient(total_loss, var_list), var_list))
        ...
        if tf.equal(tf.math.floormod(i, self.critic_frequency), 0):
            ...
            with tf.GradientTape() as tape:
                ...
                # evaluate the discriminator on generated samples
                x_fake = self.generator.sample(y_real, temp=self.temp, training=True)
                p_fake, d_fake, acc_fake = self.discriminator.loss(
            ...
            self.generator_optim.apply_gradients(zip(tape.gradient(total_loss, var_list), var_list))
    ...
    def launch(self, train_data, validate_data, logger, epochs, start_epoch=0, header=""):
        ...
        for e in range(epochs):
            self.temp.assign(
                self.final_temp * e / (epochs - 1) + self.start_temp * (1.0 - e / (epochs - 1))
            )
            for name, loss in self.train(train_data).items():
